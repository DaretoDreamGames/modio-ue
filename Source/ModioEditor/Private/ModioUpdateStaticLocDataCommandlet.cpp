...
int UModioUpdateStaticLocDataCommandlet::Main(const FString& Params)
{
	...
	if (const UModioSettings* PluginSettings = GetDefault<UModioSettings>())
	{
		if (UObject* MaybeStringTable = PluginSettings->LocalizationStringTable.TryLoad())
		{
			...
			// EFileSizeUnit is a non-`enum class` UENUM, which UE 5.8 no longer provides StaticEnum<>() for;
			// resolve its reflected enum at runtime and use the UEnum* overload instead.
			ModioUI::RegisterEnumAsLocalizable(StaticLocDataTable,
											   FindObject<UEnum>(nullptr, TEXT("/Script/Modio.EFileSizeUnit")));
